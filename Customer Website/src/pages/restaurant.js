...
export default function Menu() {
  ...
  const productsList = useSelector((state) => state.productListByRestaurant)
  const { loading, error, restaurantProducts:meals, page, pages } = productsList
  let { id } = useParams();
  let location = useLocation();
  let [color, setColor] = useState("#39B54A");
  
  useEffect(() => {
    dispatch(listProductByRestaurants("","",id))
  }, [
    dispatch,
    useParams,
    location
          ...
          <div className="lg:col-start-1 lg:col-end-6 w-full">
            ...
            <section className="">
              ...
              <div className="flex flex-col w-full mx-auto items-center xs:grid justify-center grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-y-6">
                {meals.map((item, index)=> <MenuLOCard key={index} id={item?._id} price={item?.price} name={item?.name} restaurant={item?.restaurant?.restaurant} img={item?.productImages[0]}/> )}
              </div>
              {/* <LatestOrder /> */}
